...
Shader "UGameTech/Object_N_frag"
{
	Properties
	{
		...
		_SpecularColor("Specular Color", Color)				= (0.1, 0.1, 0.1, 0.1)

		_TESTElectric("debug", Float)						= 0.0
    ...
    SubShader
	{
        ...
        Pass
		{
			...
			float4	ProcessElectric( float3 viewDir, float3 normal, float2 uv )
			{
				...
				fResult.rgb = (Multiply2.rgb * _ElectricClr.a);
